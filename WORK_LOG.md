...
## 2026-09-10 - v1.6.5 图库新增 6 张照片上线

- 核对 6 张来源文件的文件名、哈希、尺寸、色彩空间与拍摄日期，确认与现有 60 张图库没有重复。
- 将原图保存在被 Git 忽略的 `images/originals/`，生成 Web、Medium 与 Thumbnail 三档公开资源，图库总数由 60 增至 66。
- 在 `scripts/build-gallery.ps1` 增加 6 条稳定排序种子，并让新照片可以使用明确的拍摄日期，避免以后重建时被文件导出日期覆盖。
- 同步更新 `content/photos.json`、图库数量和新增照片回归契约；正式版本更新为 `v1.6.5`，全站缓存标识统一为 `v165-gallery66`。
- 完成 JavaScript 语法、JSON 与图片尺寸契约、正式静态构建、桌面 1440 × 900 与手机 390 × 844 浏览器检查；六个新增 REL 页面均加载正确图片，无横向溢出或控制台错误。
- 发布范围仅包含新增照片成品、图库数据、生成脚本、版本与日志；不提交原图、`dist/`、Photopea 草稿或工作区原有的其他未提交改动。

## 2026-08-31 - v1.6.4 移动 Safari、首页头像与抖音入口上线

- 桌面首页头像从围绕品牌文字观察照片，改为读取悬浮照片的真实视口几何并停靠在照片附近；候选停靠点会避开可见照片、语言与主题控件以及视口边缘，并保持头像椭圆碰撞范围。
- 头像锁定照片后，页面继续滚动会同步平移头像当前位置与目标位置；无限网格回收原节点时按 `rel` 找回对应副本，避免头像停留在旧屏幕位置。真实指针移动决定新悬浮目标，页面滚动造成的伪 `pointerover` 不会误切照片。
- 桌面照片悬浮整图放大调整为 `1.035`，加入克制的阴影过渡；移动端现有按滚动稳定后选图的逻辑不变。
- 为 iPhone/iPad Safari 增加浏览器栏收起后的滚动交接：仅在检测到实际视口高度增长后锁定，REL 保留文档位置，DESIGN 将文档滚动量转移到站内滚动面；菜单和 DESIGN 预览打开时会锁定同一滚动面。
- 全站菜单抖音入口改为用户提供的公开分享短链，并使用当前页直接导航，避免登录敏感的旧固定主页链接在未登录浏览器中失效。
- 同步更新 `rel=7` 与 `rel=26` 的中英地点和随记正文，并同步修改 `scripts/build-gallery.ps1` 生成种子。
- 正式版本更新为 `v1.6.4`，共享 CSS、JS 与 Design 资源缓存标识统一为 `v164-mobile-chrome-home1`；图库保持 60 张，DESIGN 内容不变。

## 2026-08-08 - v1.6.3 REL 主图缩略图到高清图顺序上线

- 排查确认 REL 主图此前在缩略图请求开始后约 180ms 就可能启动高清图，导致线上环境中主图与轨道缩略图竞争连接和解码资源；本地预览因缓存更容易掩盖这个问题。
- 加入主图关键加载闸门：主图缩略图请求使用独立 gate，只有缩略图完成加载并解码后，才启动高清图预加载；高清图解码完成后再通过现有图片加载器无闪烁替换。
- 闸门只阻止新的非当前主图任务，不取消已有请求，不改变照片取色占位态、重试机制、后台恢复机制和轨道滑动逻辑；质量图请求失败时释放闸门并沿用原有重试。
- 增加 `photoProgressiveStage` 状态标记和回归契约，覆盖主图顺序、队列独占优先通道以及取消旧的 180ms 并行启动路径。
- 完成语法检查、站点回归、正式静态构建、diff 空白检查和发布前敏感信息检查后推送 `main`，由 Cloudflare Pages 自动部署。

## 2026-08-05 - v1.6.2 浏览器后台恢复与图库更新上线

- 排查确认浏览器在标签页进入后台后暂停 JavaScript、计时器和渲染属于正常节能策略；真正的问题是页面恢复时，旧图片请求回调、动画锁和临时图片节点仍可能停留在冻结前的状态。
- 图片加载器增加活动任务登记和请求代际校验：恢复时重启仍停滞的任务，忽略已过期回调，并根据图片实际解码状态重新同步取色占位层与可见图片，避免纯色加载态永久覆盖照片。
- REL、随记和 INDEX 增加统一恢复入口：清理冻结期间遗留的 RAF、动画与输入锁，归还 INDEX 转场借用的真实图片节点，重新校准主图清晰度、当前缩略图和索引卡片状态。
- 使用浏览器冻结与恢复测试覆盖 REL 连续切图、慢速图片请求、INDEX 进入与退出、移动端随记切图，以及主页深滚动后的照片回收；恢复后可见照片均重新显示，关键交互继续可用且无控制台错误。
- 新增 2 张照片并生成 Web、Medium 与 Thumbnail 三档资源，图库总数由 58 增至 60；公开日志只记录数量。
- 正式版本更新为 `v1.6.2`，共享 CSS、JS 与 Design 资源缓存标识统一为 `v162-resume-recovery1`；生产构建继续排除摄影原图、PSD、本地 Photopea 草稿和本地文件路径。
