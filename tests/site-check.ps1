...
Assert-True ($UpdateLog -match "v1\.1\.2" -and $UpdateLog -match "RELEASE_LOG\.md" -and $UpdateLog -match "WORK_LOG\.md") "update log should act as the versioned log index"
Assert-True ($ReleaseLog -match "v1\.1\.2" -and $ReleaseLog -match "v1\.1\.1" -and $ReleaseLog -match "v1\.1\.0" -and $ReleaseLog -match "v1\.0\.1" -and $ReleaseLog -match "v1\.0\.0" -and $ReleaseLog -match "no-index-release1" -and $ReleaseLog -match "data-focus-index") "formal release log should describe the current version against the previous official release"
...
Assert-True ($LogsHtml -match '<meta charset="utf-8">' -and $LogsHtml -match 'new TextDecoder\("utf-8"\)' -and $LogsHtml -match "WORK_LOG\.md" -and $LogsHtml -match "data-log-key") "browser log preview should force UTF-8 for local markdown viewing"
...
Assert-True ($AllText -match "no-index-release1" -and $UpdateLog -match "logs-history-fix1" -and $ReleaseLog -match "logs-history-fix1") "non-index release should be versioned and logged"
