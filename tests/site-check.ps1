...
Assert-True ($Css -match "\.nav-screen__avatar[\s\S]*object-fit:\s*contain" -and $Css -match "body\[data-theme=`"dark`"\]\s+\.nav-screen__avatar") "menu avatar should display as an uncropped line/avatar asset and adapt in dark theme"
...
Assert-True ($App -match "MOTION_LOW_PASS_RATE" -and $App -match "ORIENTATION_LOW_PASS_RATE" -and $App -match "blendMotion\(gamma / 38,[\s\S]*ORIENTATION_LOW_PASS_RATE" -and $App -match "blendMotion\(x, y,[\s\S]*MOTION_LOW_PASS_RATE") "mobile home gravity should low-pass filter device orientation and motion before applying it to the letter physics"
Assert-True ($App -match "motionWakeStrength" -and $App -match "motionWakeStrength > 0\.08" -and $App -match "piece\.sleeping = false;[\s\S]*piece\.vx \+= motionInfluence\.x") "mobile device tilt should wake resting home letters instead of being ignored after they settle"
...
Assert-True ($App -match "ensureFocusMainSwipeLayer" -and $App -match "focusMainSwipe" -and $App -match "startFocusMainSwipeAnimation" -and $App -match "cancelFocusMainSwipeAnimation" -and $App -match "focusMainTouchVelocityX") "mobile focus main image should own an isolated horizontal swipe layer with interruptible animation state and hand velocity"
Assert-True ($App -match "setFocus\(targetIndex, true, \{ instant: true, source: `"swipe`" \}\)" -and $App -match "glideFocusRailAfterThumbClick\(targetIndex\)" -and $App -match "Math\.sign\(offset\)" -and $Css -match "\.focus-swipe-layer" -and $Css -match "\.focus-shell\.is-main-swiping" -and $Css -match "\.focus-shell\.is-main-swipe-animating") "mobile rel horizontal swipes should move only one image, animate current/next photos, and sync the thumbnail rail"
...
Assert-True ($App -match "handleFocusMainPointerDown" -and $App -match "handleFocusMainPointerMove" -and $App -match "handleFocusMainPointerUp" -and $App -match "pointerType !== `"mouse`" && event\.pointerType !== `"pen`"" -and $App -match "setPointerCapture" -and $App -match "releasePointerCapture") "desktop preview should support mouse and pen dragging through pointer events without double-triggering touch"
Assert-True ($App -match "handleFocusMainWheel" -and $App -match "imageToggle\.addEventListener\(`"wheel`", handleFocusMainWheel" -and $App -match "Math\.abs\(event\.deltaX\)" -and $App -match "startFocusMainSwipeAnimation\(targetOffset, !isEdgeSwipe\)" -and $App -match "Math\.max\(430" -and $App -match "Math\.min\(760") "focus main image should show a visible slide-in/slide-out transition for trackpad wheel swipes, separate from thumbnail clicks"
Assert-True ($App -match "--focus-swipe-progress" -and $Css -match "--focus-swipe-current-opacity" -and $Css -match "--focus-swipe-next-opacity" -and $Css -match "--focus-swipe-current-scale" -and $Css -match "--focus-swipe-next-scale" -and $Css -match "\.focus-swipe-layer \.focus-swipe-image\s*\{[\s\S]*transition:\s*none" -and $Css -match "\.focus-swipe-layer \.focus-swipe-image--current" -and $Css -match "\.focus-swipe-layer \.focus-swipe-image--next" -and $App -match "Math\.max\(430" -and $App -match "Math\.min\(760") "focus main slide transition should be visually legible and frame-controlled, not delayed by the base image transition"
...
Assert-True ($App -match "width \+ gap" -and $App -match "currentOpacity = isEdge \? 1 : Math\.max\(0, 1 - progress\)" -and $App -match "nextOpacity = isEdge \? 0 : nextFade" -and $App -match "currentScale = isEdge \? Math\.max\(0\.975" -and $App -match "nextScale = isEdge \? 1 : Math\.min\(1, 0\.94") "focus main swipe should separate normal photos with gap/fade/scale while keeping edge bounce subtle"
...
Assert-True ($App -match "setFocusManualSelection\(targetIndex, 1300\)" -and $App -match "focusMainSwipeLockUntil = Date\.now\(\) \+ 1300" -and $App -match "holdFocusRailForMainSwipeGesture" -and $App -match "const currentIndex = Number\(shell\.dataset\.activeIndex \|\| 0\)" -and $App -match "setFocusManualSelection\(currentIndex, duration\)" -and $App -notmatch "function handleFocusMainTouchStart[\s\S]{0,900}focusMainSwipeLockUntil = 0") "main-image swipe should cancel the thumbnail glide but keep the current image pinned so fast repeated swipes are not stolen by rail sync"
Assert-True ($App -match "function isFocusMainSwipeBusy" -and $App -match "const syncPaused = isFocusMainSwipeBusy\(\) \|\| manualSelected" -and $App -match "focusMainWheelLockUntil = Date\.now\(\) \+ 180" -and $App -notmatch "focusMainWheelLockUntil = Date\.now\(\) \+ 520") "main-image swipe should pause rail-based syncing while the main image is busy and avoid a long wheel lock that blocks fast swipes"
Assert-True ($App -match "interruptFocusMainSwipeForNewGesture" -and $App -match "handleFocusMainTouchStart[\s\S]*interruptFocusMainSwipeForNewGesture\(\)" -and $App -match "handleFocusMainPointerDown[\s\S]*interruptFocusMainSwipeForNewGesture\(\)" -and $App -match "handleFocusMainWheel[\s\S]*interruptFocusMainSwipeForNewGesture\(\)") "main-image swipe should let a new gesture interrupt an unfinished swipe handoff"
...
Assert-True ($UpdateLog -match "v1\.1\.3" -and $UpdateLog -match "RELEASE_LOG\.md" -and $UpdateLog -match "WORK_LOG\.md") "update log should act as the versioned log index"
Assert-True ($ReleaseLog -match "v1\.1\.3" -and $ReleaseLog -match "v1\.1\.2" -and $ReleaseLog -match "v1\.1\.1" -and $ReleaseLog -match "v1\.1\.0" -and $ReleaseLog -match "v1\.0\.1" -and $ReleaseLog -match "v1\.0\.0" -and $ReleaseLog -match "no-index-release1" -and $ReleaseLog -match "data-focus-index") "formal release log should describe the current version against the previous official release"
...
Assert-True ($App -match "v1\.1\.3" -and $App -match "v1\.1\.2" -and $App -match "v1\.1\.1" -and $App -match "v1\.1\.0" -and $App -match "v1\.0\.1" -and $App -match "v1\.0\.0") "logs data should list all formal public versions"
...
Assert-True ($AllText -match "stable-swipe-release1" -and $LogsHtml -match "stable-swipe-release1" -and $UpdateLog -match "stable-swipe-release1" -and $ReleaseLog -match "stable-swipe-release1") "current preview assets should use a fresh cache key while release logs keep the current deployed version"
